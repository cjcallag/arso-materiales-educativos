---
title: "Datos Relacionales de Modo Uno"
...
*"... es una colección de teorías y métodos que suponen que el comportamiento de los actores (por ejemplo, individuos, grupos u organizaciones) se ve profundamente afectado por sus vínculos con otros y las redes en las que están incrustados"*

.pull-right[
-Cunningham, Everton, y Murphy, 2016
]
...
--
.center[
**No es software!** Pero lo utilizamos.
]
...
- Medios sociales (ejemplo, Twitter, Facebook, VK, TikTok, etc.)


---

# Medios Sociales, ¿qué son?

```{r, echo = FALSE}
# datos_sendero <- rtweet::search_tweets(q = "sendero luminoso",
#                                        n = 18000,
#                                        include_rts = TRUE)
# saveRDS(datos_sendero, file = "datos_sendero.RDS")
datos_sendero <- readRDS("datos_sendero.RDS")
```

- Diferentes tipos de plataformas de medios sociales:
    - Sitios de microblogs: Twitter, Weibo, etc.
    - Sitios de comunidad: Facebook, RenRen, VK, etc.
    
- ¿Qué tipo de información se puede encontrar en las redes sociales?
    - Datos estructurados: Relaciones y atributos, pertenencia a grupos, hashtags, etc.
    - Datos no estructurados: Noticias (texto), sentimientos (palabras), emojis, imágenes, videos, etc.
...
<center>
[Haga clic aquí](https://twitter.com/search?q=sendero%20luminoso&src=typed_query)
<center/>
...
datos_sendero %>%
  ...
  labs(x = "", y = "# de Tweets", title = "Volumen de tweets recientes con palabra clave: 'Sendero Luminoso' \n")
  
```

---

# Usuarios Activos: Twitter

```{r}
datos_sendero[is_retweet == FALSE, .N, by = screen_name][order(-N)] %>%
  head(10) %>%
  knitr::kable(format = "html", col.names = c("Cuenta", "# de Tweets"), 
               caption = "10 usuarios más activos")
```
...
spatial_df <- rtweet::lat_lng(datos_sendero)
sf::st_as_sf(spatial_df[!is.na(spatial_df$lat), ],
             coords = c("lng", "lat"),
             crs = sf::st_crs("WGS84")) %>%
  ggplot() +
  ...
  geom_sf() +
  theme_minimal() + 
  theme(panel.background = element_rect(fill = "#daf3ff")) +
  labs(title = "Ubicación de tweets recientes con palabra clave: 'Sendero Luminoso' \n")
  
```

---

# Extracción de textos: Twitter

```{r, fig.align='center', fig.height=6}
library(tm)
corpus <- SimpleCorpus(VectorSource(datos_sendero$text))
corpus <- tm_map(corpus, content_transformer(tolower))
corpus <- tm_map(corpus, removeNumbers)
corpus <- tm_map(corpus, removePunctuation)
corpus <- tm_map(corpus, removeWords, stopwords("spanish"))
corpus <- tm_map(corpus, removeWords, c("sendero", "luminoso"))

DTM <- DocumentTermMatrix(corpus)
sums <- as.data.frame(colSums(as.matrix(DTM)))
sums <- tibble::rownames_to_column(sums) 
colnames(sums) <- c("term", "count")
sums <- dplyr::arrange(sums, dplyr::desc(count))
head <- sums[1:75,]
wordcloud::wordcloud(words = head$term,
                     freq = head$count,
                     min.freq = 1000,
                     max.words=100,
                     random.order=FALSE,
...
```{r, fig.align='center', fig.height=6}
reply_to <- datos_sendero[, .("from" = screen_name,
                              "to"   = reply_to_screen_name,
                              "type" = "Reply")]
mentions <- tidyr::unnest(datos_sendero[, .("from" = screen_name,
                                            "to"   = mentions_screen_name, 
                                            "type" = "Mentions")])
rbind(reply_to, mentions) %>%
  ...
  plot.igraph(
    ...
    vertex.label = NA,
    vertex.color = "black",
    edge.color = ifelse(E(.)$type == "Mentions", "red", "blue"),
    edge.arrow.size = 0.5,
...
**Estándar**
  ...
  1. ¿Qué miembros de un grupo se adscriben a una doctrina? ¿Cuántos hay en comparación con los que no se adscriben?
  2. ¿Cómo caracterizamos a un grupo? ¿Utilizando estadística descriptiva?
...
.pull-right[
**Relacional**
  1. ¿En qué parte del grupo se encuentran los miembros que se adscriben a una doctrina? ¿Son los centrales a el grupo?
  2. ¿Cuáles son las características estructurales de un grupo? Descripción desde la perspectiva:
    - Macro
    - Meso
    - Micro
]
...
# Hola [snExplorer](https://corelab.nps.edu/apps/sn_explorer/)!

```{r, fig.align='center'}
knitr::include_graphics(path = "imgs/corelogo.png")
```
...
<center/>
**Instrucciones:** Con una sola matriz, capture todas las relaciones en el siguiente corpus. Su trabajo por ahora es capturar las relaciones, no preocuparse por categorizarlas (lo haremos más adelante). Además, no se preocupe por los pesos de los bordes. Finalmente, suponga que todas las relaciones son recíprocas.
...
# Repaso
  ...
  1. ¿Cuáles son las diferencias entre los datos de mode uno y modo dos?
